"""
Authentication middleware for TMS Service
"""
...
class AuditLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        Returns:
            HTTP response
        """
        if not getattr(settings, 'enable_audit_trail', True):
            return await call_next(request)
        ...
        logger.info(f"TMS API Request: {request.method} {request.url.path}", extra={"audit_data": audit_data})
        ...
        try:
            ...
            logger.info(
                f"TMS API Response: {response.status_code} in {duration:.3f}s",
            ...
            logger.error(
                f"TMS API Error: {str(e)} in {duration:.3f}s",
                extra={"audit_data": audit_data}
            )
