"""
JWT Authentication utilities for TMS Service
"""
...
def log_authentication_event(
    ...
    if success:
        logger.info(f"TMS Auth event: {event_type}", extra={"auth_data": log_data})
    else:
        logger.warning(f"TMS Auth event: {event_type} - {reason}", extra={"auth_data": log_data})
...
def verify_token(token: str) -> TokenData:
    ...
    try:
        # Add debug logging
        logger.debug(f"TMS: Attempting to decode token with algorithm: {settings.GLOBAL_JWT_ALGORITHM}")
        logger.debug(f"TMS: JWT secret (first 10 chars): {settings.GLOBAL_JWT_SECRET[:10]}...")
        ...
        logger.debug(f"TMS: Token payload - user_id: {user_id}, tenant_id: {tenant_id}, role_id: {role_id}, role: {role}")

        if user_id is None or role_id is None:
            logger.warning("TMS: Token missing required fields")
        ...
        logger.warning(f"TMS: JWT validation failed: {str(e)}")
        logger.warning(f"TMS: Token received (first 20 chars): {token[:20]}...")
        if "expired" in str(e).lower():
            raise TokenExpiredError("Token has expired")
        raise TokenInvalidError(f"Invalid token: {str(e)}")
    except Exception as e:
        logger.warning(f"TMS: Unexpected error during token validation: {str(e)}")
