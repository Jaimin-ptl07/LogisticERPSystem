...
@router.get("/status-counts", response_model=DriverStatusCountsResponse)
async def get_driver_status_counts(
    ...
    try:
        ...
        status_counts = [
            DriverStatusCount(status=row[0], count=row[1])
            for row in rows
        ]

        total_drivers = sum(sc.count for sc in status_counts)

        return DriverStatusCountsResponse(
