-- Migration 024: Add columns for 4 key features to company service
...
COMMENT ON COLUMN vehicles.last_odometer_update IS 'Timestamp of the last odometer update';
...
COMMENT ON
TABLE vehicle_odometer_fuel_logs IS 'Logs for tracking vehicle odometer readings and fuel economy over time';
...
COMMENT ON
TABLE product_unit_types IS 'Reference table for product measurement units (e.g., kg, liters, pieces)';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_driver_code_tenant ON driver_profiles (driver_code, tenant_id)
WHERE
    driver_code IS NOT NULL;
