...
const orderFormSchema = z.object({
  orderNumber: z.string().optional(), // Backend generates order number in DDMMYYYY-{sequence} format
...
export function CreateOrderModal({
  ...
  useEffect(() => {
    if (isEditMode && order) {
      ...
      // Order number will be generated by backend in DDMMYYYY-{sequence} format
      // No need to set it here - backend will auto-generate
    }
  ...
  const onSubmit = async (data: OrderFormData) => {
    try {
      ...
      // Only include order_number if it has a value (for edit mode or manual override)
      if (data.orderNumber) {
