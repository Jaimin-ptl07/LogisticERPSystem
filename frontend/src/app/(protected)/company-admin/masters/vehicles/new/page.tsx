...
export default function NewVehiclePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/masters/vehicles/${newVehicle.id}`);
    } catch (error: any) {
      console.error('Vehicle creation error:', error);

      // Show the actual backend error message directly in toast
      const errorMessage = error?.data?.detail || error?.message || 'Failed to create vehicle';
      ...
      if (typeof errorMessage === 'string') {
        ...
        if (lowerMessage.includes('plate number already exists') || lowerMessage.includes('vehicle with this plate number already exists')) {
          setErrors(prev => ({
            ...prev,
