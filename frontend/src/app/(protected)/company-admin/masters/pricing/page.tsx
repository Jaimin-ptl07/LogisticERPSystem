...
export default function PricingPage() {
  ...
  const calculatePrice = (basePrice: number, distance: number, weight: number, fuelSurcharge: number) => {
    const distanceCost = distance * 0.5; // Mock calculation
    const weightCost = weight * 0.1; // Mock calculation
    const fuelCost = (basePrice + distanceCost + weightCost) * (fuelSurcharge / 100);
  ...
  return (
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 mt-2">Manage your pricing rules and service rates</p>
          </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{pricingRules?.length || 0}</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">
                    ${pricingRules?.reduce((sum, r) => sum + r.base_price, 0) ?
                      (pricingRules.reduce((sum, r) => sum + r.base_price, 0) / (pricingRules.length || 1)).toFixed(0) : '0'}
                  </p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600">{serviceTypes.length}</p>
                </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-4">
                  {searchQuery ? 'Try adjusting your search' : 'Get started by creating your first pricing rule'}
                </p>
                {!searchQuery && (
                ...
                <TableBody>
                  {filteredRules.map((rule) => (
                    <React.Fragment key={rule.id}>
                      <TableRow className="hover:bg-gray-50 cursor-pointer" onClick={() => toggleRowExpansion(rule.id)}>
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            {expandedRows.has(rule.id) ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
                            <span className="font-medium">{rule.name}</span>
                          </div>
                        </TableCell>
                        <TableCell>{getServiceTypeBadge(rule.service_type || '')}</TableCell>
                        <TableCell>
                          <div className="flex items-center text-sm text-gray-900">
                            <MapPin className="w-4 h-4 mr-2" />
                            {rule.zone_origin || 'Any'} → {rule.zone_destination || 'Any'}
                          </div>
                        </TableCell>
                        <TableCell className="font-medium">${rule.base_price.toFixed(2)}</TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={9} className="bg-gray-50">
                            <div className="p-4 space-y-4">
                              <div>
                                <h4 className="font-semibold text-gray-900 mb-2">Price Calculation Example</h4>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-lg font-bold text-blue-600">
                                      ${calculatePrice(rule.base_price, 100, 50, rule.fuel_surcharge_percent).toFixed(2)}
                                    </p>
                                  </div>
                                </div>
                              </div>
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-700">Origin Zone</Label>
                  <select
                    value={formData.zone_origin}
                    onChange={(e) => setFormData({ ...formData, zone_origin: e.target.value })}
                    className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  <Label className="text-sm font-medium text-gray-700">Destination Zone</Label>
                  <select
                    value={formData.zone_destination}
                    onChange={(e) => setFormData({ ...formData, zone_destination: e.target.value })}
                    className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <option value="">All Zones</option>
                    {zones.map(zone => (
