  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        ...
        {(() => {
          const activeTrip = allTrips.find(trip => trip.status === 'on-route' || trip.status === 'loading');
            ...
            <div className="border-2 border-blue-500 rounded-lg p-6 mb-8 bg-blue-50">
              <div className="mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-black">
                  ...
                  <div>
                    <span className="font-medium">Progress:</span> {activeTrip.completed_orders}/{activeTrip.order_count} Orders
                  </div>
                  <div>
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            ) : allTrips.filter(trip => trip.status !== 'on-route' && trip.status !== 'loading').length === 0 ? (
              <div className="text-center py-12">
                <Truck className="w-16 h-16 mx-auto mb-4 text-gray-300" />
                <h3 className="text-lg font-medium text-black mb-2">No other trips found</h3>
                <p className="text-black">
                  All your trips are currently active
                </p>
              </div>
            ) : (
              allTrips.filter(trip => trip.status !== 'on-route' && trip.status !== 'loading').map(trip => {
                const tripDetail = tripDetails.get(trip.id);
                const tripOrders = tripDetail?.orders || [];

                return (
                  <div key={trip.id} className="border rounded-lg p-6 bg-gray-50">
                    <div className="mb-4">
                      <h3 className="text-lg font-bold text-black mb-2">
                        {trip.truck_plate} - {trip.origin || 'N/A'} → {trip.destination || 'N/A'}
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-black">
                        <div>
                          <span className="font-medium">Trip ID:</span> {trip.id}
                        </div>
                        <div>
                          <span className="font-medium">Status:</span>{' '}
                          <span className={`ml-1 px-2 py-1 rounded text-xs text-white ${
                            trip.status === 'completed' ? 'bg-green-600' :
                            trip.status === 'planning' ? 'bg-gray-600' :
                            'bg-gray-600'
                          }`}>
                        ...
                        <div>
                          <span className="font-medium">Progress:</span> {trip.completed_orders}/{trip.order_count} Orders
                        </div>
                        <div>
                          <span className="font-medium">Date:</span> {new Date(trip.trip_date).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                    ...
                    {tripOrders.length > 0 && (
                      <div className="border-t border-gray-300 pt-4">
                        <h4 className="text-lg font-semibold mb-3 text-black">Orders ({tripOrders.length})</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {tripOrders.map(order => (
                            <div key={order.id} className="border rounded p-3 bg-white">
                              <div className="flex justify-between items-center mb-2">
                                <h5 className="font-black text-black">Order #{order.sequence_number + 1} - {order.order_id}</h5>
                                <span className={`px-2 py-1 rounded text-xs font-medium text-white ${
                                  order.delivery_status === 'delivered' ? 'bg-green-600' :
                                  'bg-gray-600'
                                }`}>
                                  {order.delivery_status === 'delivered' ? 'DELIVERED' :
                                   order.delivery_status.toUpperCase().replace('-', ' ')}
                                </span>
                              </div>
                              ...
                              <p className="text-sm text-black mb-1">
                                <span className="font-medium">Items:</span> {order.items} items ({order.weight} kg)
                              </p>
                              {order.delivery_status === 'delivered' && (
